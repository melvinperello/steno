...
3. **Batch UX Polish**:
   - Resolved an issue where running directories (e.g., 79 files) caused massive terminal clutter by stacking dead UI panels.
   - *Fix*: Enabled `transient=True` on `rich.live.Live` to evaporate finished panels, leaving only a clean 1-line log of completed files.

4. **ONNX Warning Suppression**:
   - Silenced native Python `UserWarning` exceptions generated by `onnxruntime` when evaluating hardware providers, preventing them from glitching the terminal output between files.
